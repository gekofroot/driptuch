...
let irisSelect = document.getElementById('iris-select')
...
let boosterItemWrapper = document.getElementById('booster-item-wrapper')
let orbItemWrapper = document.getElementById('orb-item-wrapper')
let fieldAreas = document.getElementById('field-areas')
let pointsFieldArea = document.getElementById('points-field-area')
let boosterField = document.getElementById('booster-field')
let dropletsFieldArea = document.getElementById('droplets-field-area')
...
let storedIris = localStorage.getItem('iris')
...
let irisValues = [
  '--iris-almond', '--iris-rose',
  '--iris-vert', '--iris-vio',
  '--iris-bleu'
]

let pupilValues = [
  '--pupil-almond', '--pupil-rose',
  '--pupil-vert', '--pupil-vio',
  '--pupil-bleu'
]

if (storedIris) {
  irisSelect.value = storedIris
  for (let x = 0; x < irisType.length; x++) {
    if (storedIris === irisType[x]) {
      pointsDisplay.style.background = `radial-gradient(var(${irisValues[x]}), var(--acnt-inset))`
      pointsDisplay.style.color = `var(${pupilValues[x]})`
      dropletsDisplay.style.background = `radial-gradient(var(${irisValues[x]}), var(--acnt-inset))`
      dropletsDisplay.style.color = `var(${pupilValues[x]})`
    }
  }
} else {
  document.documentElement.setAttribute("data-theme", 'Peachy')
  localStorage.setItem('iris', 'Rose')
  themeSelect.value = 'Rose'
  for (let x = 0; x < irisType.length; x++) {
    if (themeSelect.vale === irisType[x]) {
      pointsDisplay.style.background = `radial-gradient(var(${irisValues[x]}), var(--acnt-inset))`
      pointsDisplay.style.color = `var(${pupilValues[x]})`
      dropletsDisplay.style.background = `radial-gradient(var(${irisValues[x]}), var(--acnt-inset))`
      dropletsDisplay.style.color = `var(${pupilValues[x]})`
...
for (let x = 0; x < 7; x++) {
  ...
  orbItemWrapper.appendChild(orbItem)
}

for (let x = 0; x < orbItemWrapper.children.length; x++) {
  orbItemWrapper.children[x].style.background = 'var(--orb-inactive)'
}
...
for (let x = 0; x < 4; x++) {
  ...
  boosterItemWrapper.appendChild(orbItem)
}
for (let x = 0; x < boosterItemWrapper.children.length; x++) {
  boosterItemWrapper.children[x].style.background = 'var(--booster-inactive)'
}

// area anim
// field area anim
function fieldAreaAnim() {
  pointsFieldArea.style.padding = 'unset'
  pointsFieldArea.style.height = '0'
  dropletsFieldArea.style.padding = 'unset'
  dropletsFieldArea.style.height = '0'
  mainFieldBorder.style.marginTop = '3em'
  mainFieldBorder.style.transition = '.4s'
  setTimeout(() => {
    pointsFieldArea.style.padding = '2em'
    pointsFieldArea.style.height = '3em'
    dropletsFieldArea.style.padding = '2em'
    dropletsFieldArea.style.height = '3em'
...
function dropletDrip() {
  ...
  dropletInterval = setInterval(() => {
	...
	mainField.style.border = '.1em outset var(--bd)'
	...
	if (droplets % 3 === 0) {
	  ...
	  if (orbCount < 7) {
	    ...
	    for (let x = 0; x < orbCount; x++) {
	      orbItemWrapper.children[x].style.background = 'var(--orb-active)'
	    }
	  }
	  newDroplet.style.background = `radial-gradient(${boostDripColors[boostCount]}, var(--orb-active))`
	  newDroplet.style.boxShadow = `${boostDripColors[boostCount]} 0 0 .1em .1em, inset -.2em -.2em .1em ${boostDripColors[boostCount]}`
	  ...
	  newDroplet.style.boxShadow = `var(--droplet-acnt) 0 0 .1em .1em, inset -.2em -.2em .1em var(--bd)`
	}
...
// boost drip glitter droplets (gold/silver)
...
// boost drip glitter droplets b (rainbow)
...
play.addEventListener('click', () => {
  ...
  if (gameFlag === 1) {
    ...
    setTimeout(() => {
      ...
      // game loop active
      mainField.style.border = '.1em outset var(--orb-active)'
	...
	boosterItemWrapper.children[x].style.background = 'var(--booster-active)'
      }
...
themeSelect.addEventListener('change', () => {
  ...
  for (let x = 0; x < irisType.length; x++) {
    if (irisSelect.value === irisType[x]) {
      pointsDisplay.style.background = `radial-gradient(var(${irisValues[x]}), var(--acnt-inset))`
      pointsDisplay.style.color = `var(${pupilValues[x]})`
...
irisSelect.addEventListener('change', () => {
  selectedIris = irisSelect.value
  for (let x = 0; x < irisType.length; x++) {
    if (selectedIris === irisType[x]) {
      pointsDisplay.style.background = `radial-gradient(var(${irisValues[x]}), var(--acnt-inset))`
      pointsDisplay.style.color = `var(${pupilValues[x]})`
  ...
  localStorage.setItem('iris', selectedIris)
  irisSelect.value = selectedIris
})
...
let splashInterval = setInterval(()=> {
  if (splashCount > elements.length - 1) {
    ...
    dropletsDisplay.style.display = 'flex'
    setTimeout(()=> {
      pointsDisplay.style.opacity = '100'
      dropletsDisplay.style.opacity = '100'
    }, 600)
  } else {
