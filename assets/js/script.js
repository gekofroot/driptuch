...
function dropletDrip() {
  ...
  dropletInterval = setInterval(() => {
	...
	mainFieldBorder.style.animationPlayState = 'paused'
      } else if (mainField.children.length >= 5 && mainField.children.length < 10) {
	mainFieldBorder.style.animationPlayState = 'running'
	mainFieldBorder.style.animationDuration = '5s'
      } else if (mainField.children.length >= 10 && mainField.children.length < 13) {
	mainFieldBorder.style.animationDuration = '2s'
      } else if (mainField.children.length >= 13 && mainField.children.length <= 15) {
...
mainField.addEventListener('click', () => {
  ...
  if (gameFlag === 1) {
    ...
    if (eventTarget.className === 'droplet') {
      ...
      // droplet touched
      streamCount += 1
      if (streamCount > 0 && streamCount % 70 === 0) {
	if (boosterCount < 4) {
	  displayAreaAnim(100)
	  boosterCount += 1
	  fieldAreaAnim()
	...
	if (streamCount > 0 && streamCount % 30 === 0) {
	  prevDrip(5)
	  localStorage.setItem('current-count', currentCount)
	} else if (streamCount > 0 && streamCount % 70 === 0) {
	  prevDrip(15)
	  localStorage.setItem('current-count', currentCount)
	} else if (streamCount > 0 && streamCount % 140 === 0) {
	  prevDrip(30)
	  localStorage.setItem('current-count', currentCount)
	} else if (streamCount > 0 && streamCount % 280 === 0) {
	  prevDrip(90)
	  localStorage.setItem('current-count', currentCount)
	}
      }
